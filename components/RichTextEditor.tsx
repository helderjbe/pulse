...
function RichTextEditor({ selectedDate, onContentSaved, isDatabaseReady }, ref) {
  ...
  // Single hook for managing all editor state and logic
  const { isLoadingOrSaving, handleContentChange, forceSave } = useNoteEditor({
    selectedDate,
    isDatabaseReady,
    editor,
    onContentSaved,
  ...
  // Set up event-driven content change detection using useEditorContent
  const content = useEditorContent(editor, { type: 'html' });

  useEffect(() => {
    if (!isDatabaseReady || !content) return;
    handleContentChange(content);
  }, [content, isDatabaseReady, handleContentChange]);
  ...
  return (
    <View style={[styles.container, { backgroundColor }]}>
      <View style={styles.toolbar}>
        ...
        <StatusIndicator isActive={isLoadingOrSaving} />
      </View>
